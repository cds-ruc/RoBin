...
#include "./src/include/Tree.cpp"
#include "./src/include/Tree.h"
#include <utility>

template <class KEY_TYPE, class PAYLOAD_TYPE>
class ARTInterface final
...
public:
  ...
  size_t scan(KEY_TYPE key_low_bound, size_t key_num,
    ...
    size_t resultCount;
    Key continueKey;
    idx->lookupRange(startKey, maxKey, continueKey, results, key_num,
                     resultCount);
  ...
  static void loadKey(TID tid, Key &key) {
    ...
    std::pair<KEY_TYPE, PAYLOAD_TYPE> *valPtr =
        reinterpret_cast<std::pair<KEY_TYPE, PAYLOAD_TYPE> *>(tid);
    KEY_TYPE reversed = swap_endian(valPtr->first);
    key.set(reinterpret_cast<char *>(&reversed), sizeof(valPtr->first));
  }

  ~ARTInterface() { delete idx; }
...
template <class KEY_TYPE, class PAYLOAD_TYPE>
void ARTInterface<KEY_TYPE, PAYLOAD_TYPE>::bulk_load(
...
template <class KEY_TYPE, class PAYLOAD_TYPE>
bool ARTInterface<KEY_TYPE, PAYLOAD_TYPE>::get(KEY_TYPE key,
                                                             PAYLOAD_TYPE &val,
                                                             Param *param) {
  Key k;
  ...
  auto valPtr =
      reinterpret_cast<std::pair<KEY_TYPE, PAYLOAD_TYPE> *>(idx->lookup(k));
  if (valPtr) {
    val = valPtr->second;
    return true;
  } else {
...
template <class KEY_TYPE, class PAYLOAD_TYPE>
bool ARTInterface<KEY_TYPE, PAYLOAD_TYPE>::put(KEY_TYPE key,
                                                             PAYLOAD_TYPE value,
                                                             Param *param) {
  Key k;
...
template <class KEY_TYPE, class PAYLOAD_TYPE>
bool ARTInterface<KEY_TYPE, PAYLOAD_TYPE>::update(
    KEY_TYPE key, PAYLOAD_TYPE value, Param *param) {
  Key k;
  auto temp = new std::pair<KEY_TYPE, PAYLOAD_TYPE>(key, value);
  KEY_TYPE reversed = swap_endian(key);
  k.set(reinterpret_cast<char *>(&reversed), sizeof(key));
...
template <class KEY_TYPE, class PAYLOAD_TYPE>
bool ARTInterface<KEY_TYPE, PAYLOAD_TYPE>::remove(KEY_TYPE key,
                                                                Param *param) {
  Key k;
  KEY_TYPE reversed = swap_endian(key);
  k.set(reinterpret_cast<char *>(&reversed), sizeof(key));
