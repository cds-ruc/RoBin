...
class pgmInterface final : public indexInterface<KEY_TYPE, PAYLOAD_TYPE> {
public:
...
template <class KEY_TYPE, class PAYLOAD_TYPE>
void pgmInterface<KEY_TYPE, PAYLOAD_TYPE>::bulk_load(
    std::pair<KEY_TYPE, PAYLOAD_TYPE> *key_value, size_t num, Param *param) {
  index = new pgm::DynamicPGMIndex<KEY_TYPE, PAYLOAD_TYPE>(key_value,
                                                           key_value + num);
}

template <class KEY_TYPE, class PAYLOAD_TYPE>
bool pgmInterface<KEY_TYPE, PAYLOAD_TYPE>::get(KEY_TYPE key, PAYLOAD_TYPE &val,
                                               Param *param) {
...
template <class KEY_TYPE, class PAYLOAD_TYPE>
bool pgmInterface<KEY_TYPE, PAYLOAD_TYPE>::put(KEY_TYPE key, PAYLOAD_TYPE value,
                                               Param *param) {
...
template <class KEY_TYPE, class PAYLOAD_TYPE>
bool pgmInterface<KEY_TYPE, PAYLOAD_TYPE>::update(KEY_TYPE key,
                                                  PAYLOAD_TYPE value,
                                                  Param *param) {
  index->insert_or_assign(key, value);
  return true;
...
template <class KEY_TYPE, class PAYLOAD_TYPE>
size_t pgmInterface<KEY_TYPE, PAYLOAD_TYPE>::scan(
  ...
  for (scan_size = 0; scan_size < key_num && iter != index->end();
       scan_size++) {
